...
from utils.extract_sections import extract_sections
...
def generate_rfp_response():
    ...
    ## -------------------------------------------------
    session_keys = [
        'clientOrg', 'docTitle', 'vector_store', 'template', 'sections', 
        'section_dict', 'prepared_prompts', 'generated_resp', 'doc_generated', 
        'bio', 'case_studies'
    ]

    for key in session_keys:
        if key not in st.session_state:
            st.session_state[key] = None if key in ['sections', 'section_dict', 'prepared_prompts', 'generated_resp', 'bio', 'case_studies'] else ''
                ...
                if st.session_state['sections']:
                    ...
                    if new_sections or deleted_sections:
                        ...
                        if section_dict:
                            ...
                            if on:
                                default_synoptek_sections = ['SYNOPTEK OVERVIEW', 'SYNOPTEK\'s CULTURE AND APPROACH TO TALENT MANAGEMENT', 'CASE STUDIES', 'QUALITY SECURITY AND COMPLIANCE', 'ASSUMPTIONS AND CLIENT RESPONSIBILITIES']
                                sections = st.multiselect(label='## **What sections to be included?**', options=default_synoptek_sections, default=default_synoptek_sections, key='sections0')
                                ...
                                logger.info("Section dictionary updated")

                    if st.button('Build RFP Response', key='button2'):
                        st.session_state['section_dict'] = section_dict

                if st.session_state['section_dict']:
                    prepared_prompts = prepare_prompts(sections=st.session_state['section_dict'], resp_template=st.session_state['template'], llm_resp=azure_qa)
                    ...
                    if st.session_state['prepared_prompts']:
                        generated_resp = generate_response(vector_store=st.session_state['vector_store'], llm_qa=azure_qa, llm_resp=azure_resp, prompts=prepared_prompts, clientOrg=st.session_state['clientOrg'], case_studies=st.session_state['case_studies'])
    ...
    if st.session_state['generated_resp']:
        try:
            ...
            cleaned_rfp_resp = {key: re.sub(r'\[doc \d\.\d-\d\]', '', str(values)) for key, values in st.session_state['generated_resp'].items()}
            ...
            save_dict_with_markdown_to_word(dictionary=cleaned_rfp_resp, file_path=r'./output_files/' + st.session_state['clientOrg'] + '.docx', title=st.session_state['docTitle'].upper())
