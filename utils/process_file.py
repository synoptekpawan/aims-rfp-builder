...
def process_file(file_name, file, bytes_data):
    ...
    if '.pdf' in file_name:
        # read pdf file
        with open(r'./temp/'+file_name, mode='wb') as w:
            w.write(file.getvalue())
        loader = PyMuPDFLoader(r'./temp/'+file_name)
        pages_ = loader.load()
    ...
    if ".xlsx" in file_name:
        with open(r'./temp/'+file.name, mode='wb') as w:
            w.write(file.getvalue())
        loader = UnstructuredExcelLoader(r'./temp/'+file.name,mode="paged")
        pages_ = loader.load()
    ...
    if ".docx" in file_name:
        with open(r'./temp/'+file.name, mode='wb') as w:
            w.write(file.getvalue())
        loader = Docx2txtLoader(r'./temp/'+file.name)
        pages_ = loader.load()
        pages_ = [page.page_content.lower() for page in pages_]
        # pages_ = [page.replace("aetion, inc. - confidential","") for page in pages_]
        pages_ = [page.replace("’","'") for page in pages_]
    ...
    if ".txt" in file_name:
        with open(r'./temp/'+file.name, mode='wb') as w:
            w.write(file.getvalue())
        loader = TextLoader(r'./temp/'+file.name, encoding = 'UTF-8')
        pages_ = loader.load()
        pages_ = [page.page_content.lower() for page in pages_]
        # pages_ = [page.replace("aetion, inc. - confidential","") for page in pages_]
        pages_ = [page.replace("’","'") for page in pages_]
    ...
    if ".md" in file_name:
        with open(r'./temp/'+file.name, mode='wb') as w:
            w.write(file.getvalue())
        loader = TextLoader(r'./temp/'+file.name)
        pages_ = loader.load()
        pages_ = [page.page_content.lower() for page in pages_]
        # pages_ = [page.replace("aetion, inc. - confidential","") for page in pages_]
        pages_ = [page.replace("’","'") for page in pages_]
    ...
    if ".csv" in file_name:
        with open(r'./temp/'+file.name, mode='wb') as w:
            w.write(file.getvalue())
        loader = CSVLoader(r'./temp/'+file.name)
        pages_ = loader.load()
        pages_ = [page.page_content.lower() for page in pages_]
        # pages_ = [page.replace("aetion, inc. - confidential","") for page in pages_]
        pages_ = [page.replace("’","'") for page in pages_]
